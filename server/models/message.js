...
messageDb.selectAll = (threadId, offset)=>{
    return new Promise((resolve,reject)=>{
        pool.execute(`select post_id,thread_id, message, create_date, user_name, images from posts join users on posts.user_id = users.user_id where posts.thread_id = (?) order by post_id limit 50 offset ${offset};`,[threadId],(err,results)=>{
            if(err){
                return reject(err)
            }else{
